...
class MultimediaPresenter extends Presenter
{
    ...
    {
        return new HtmlString("<button type='button' onclick='changeStatus(this);' id='btnapprove-{$this->model->id}' data-href='" . route('multimedias.change', ['id' => $this->model->id, 'status' => 'false']) . "' class='btn bg-purple waves-effect btn-sm'><i class='material-icons'  data-toggle='tooltip' data-placement='top' data-original-title='DAR DE BAJA!'>visibility</i></button>");
    }

    public function hasNotActive()
    {
        return new HtmlString("<button type='button' onclick='changeStatus(this);' id='btnreject-{$this->model->id}' data-href='" . route('multimedias.change', ['id' => $this->model->id, 'status' => 'true']) . "' class='btn bg-deep-orange waves-effect btn-sm'><i class='material-icons'  data-toggle='tooltip' data-placement='top' data-original-title='REACTIVAR'>visibility_off</i></button>");
    }
